...
function initCanvas() {
  ...
  window.addEventListener("resize", () => {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    renderUnderwaterScene(); // Re-render when window is resized
  });
}
...
function drawCoralWall(wall) {
  ...
  // Add horizontal lines for visual interest (Battlezone style)
  const maxLines = 5 + Math.floor(Math.random() * 3);
  const lineSpacing = wall.height / maxLines;

  for (let i = 1; i <= maxLines; i++) {
    const y = canvas.height - i * lineSpacing;

    // Find intersections with the wall at this height
    const intersections = findWallIntersections(wall, y);

    // Draw horizontal line segments if we found valid intersections
    if (intersections.length >= 2) {
      for (let j = 0; j < intersections.length; j += 2) {
        if (j + 1 < intersections.length) {
          ctx.beginPath();
          ctx.moveTo(intersections[j], y);
          ctx.lineTo(intersections[j + 1], y);
          ctx.stroke();
        }
      }
    }
...
// Add a perspective grid (Battlezone style)
function drawPerspectiveGrid() {
  ...
  const vanishingY = canvas.height * 0.5;

  // Draw grid lines radiating from vanishing point
  const lineCount = 7; // Odd number for center line

  for (let i = 0; i < lineCount; i++) {
    const x = (canvas.width / (lineCount - 1)) * i;
...
function renderUnderwaterScene() {
  ...
  // Generate and draw coral walls
  const coralWalls = generateCoralReef();
...
function initRenderer() {
  ...
  // For a static scene, we just need to render once
  // For animation, we would use an animation loop like this:
  // function animate() {
  //   renderUnderwaterScene();
